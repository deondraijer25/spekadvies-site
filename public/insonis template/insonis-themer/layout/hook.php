...
class GVA_Layout_Frontend_Hook extends GVA_Layout_Model{
   ...
   function __construct(){
      add_action('insonis/layouts/page', array($this, 'layout_page'));
      add_action('insonis/layouts/single/post', array($this, 'layout_single_post'));
      add_action('insonis/layouts/archive/post', array($this, 'layout_archive_post'));

      add_action('insonis/layouts/single/template', array($this, 'single_template'));

      add_action('insonis/layouts/single/product', array($this, 'layout_single_product'));
      add_action('insonis/layouts/archive/product', array($this, 'layout_archive_product'));

      add_action('insonis/layouts/single/give_form', array($this, 'layout_single_give_form'));
      add_action('insonis/layouts/archive/give_forms', array($this, 'layout_archive_give_forms'));
      
      add_action('wp_enqueue_scripts', array($this, 'insonis_themer_elementor_load_css'), 500);
   }

   public function insonis_themer_elementor_load_css(){
      ...
      if($post && $post->post_type == 'give_forms'){
         ...
         $post_meta_template = get_post_meta($post->ID, 'insonis_template_layout', true);
   ...
   public function layout_page(){
      ...
      if($post_meta_template == '_without_layout' || empty($post_meta_template) || !metadata_exists('post', $post_id, 'insonis_template_layout')){
   ...
   public function layout_single_give_form(){
      ...
      $post_id = $post->ID;
      $template_id = $this->get_template_default('donation_layout');
      $post_template_id = get_post_meta($post_id, 'insonis_template_layout', true);
   ...
   public function layout_archive_product(){
      ...
      do_action('insonis_page_breacrumb');
