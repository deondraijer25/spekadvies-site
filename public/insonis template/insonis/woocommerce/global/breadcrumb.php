...
if ( ( ! is_front_page() && ! ( is_post_type_archive() && get_option( 'page_on_front' ) == wc_get_page_id( 'shop' ) ) ) || is_paged() ) {
	...
	if ( is_home() ) {
		...
		$output_html .= $prepend . $before . esc_html__( 'Products tagged &ldquo;', 'insonis' ) . $queried_object->name . '&rdquo;' . $after;
		...
		if ( is_search() ) {
			...
			$output_html .= $before . '<a href="' . get_post_type_archive_link( 'product' ) . '">' . $_name . '</a>' . $delimiter . esc_html__( 'Search results for &ldquo;', 'insonis' ) . htmlentities(get_search_query()) . '&rdquo;' . $after;
		...
		$output_html .= $before . esc_html__( 'Error 404', 'insonis' ) . $after;
		...
		$output_html .= $before . esc_html__( 'Search results for &ldquo;', 'insonis' ) . htmlentities(get_search_query()) . '&rdquo;' . $after;

	} elseif ( is_tag() ) {

			$output_html .= $before . esc_html__( 'Posts tagged &ldquo;', 'insonis' ) . single_tag_title( '', false ) . '&rdquo;' . $after;

	} elseif ( is_author() ) {

		$userdata = get_userdata( $author );
		$output_html .= $before . esc_html__( 'Author:', 'insonis' ) . ' ' . $userdata->display_name . $after;

	}

	if ( get_query_var( 'paged' ) ) {
		$output_html .= ' (' . esc_html__( 'Page', 'insonis' ) . ' ' . get_query_var( 'paged' ) . ')';
	}
