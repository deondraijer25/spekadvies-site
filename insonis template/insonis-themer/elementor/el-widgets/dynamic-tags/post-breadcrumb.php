...
class GVAElement_Post_Breadcrumb extends GVAElement_Base{
	...
	public function get_title(){
		return esc_html__('Breadcrumb', 'insonis-themer');
	}
	...
	protected function register_controls(){
		...
		$this->add_control(
			'bg_overlay_color',
			[
				'label' => esc_html__( 'Background Overlay Color', 'insonis-themer' ),
		...
		$this->add_control(
	  	...
	  	$this->add_responsive_control(
			'padding_top',
			[
				'label' => __( 'Breadcrumb Padding Top', 'insonis-themer' ),
		...
		$this->add_responsive_control(
			'padding_bottom',
			[
				'label' => __( 'Breadcrumb Padding Bottom', 'insonis-themer' ),
				'type' => Controls_Manager::SLIDER,
				'range' => [
					'px' => [
						'min' => 0,
		...
		$this->add_control(
			'heading_content',
			[
				'label'     => __('Content & Style ------', 'insonis-themer'),
				'type'      => Controls_Manager::HEADING,
			]
	  	);
		$this->add_control(
			'show_title',
			[
				'label'     => __('Show Titlte', 'insonis-themer'),
		...
		$this->add_control(
			'title_color',
			[
				'label' => esc_html__( 'Title Color', 'insonis-themer' ),
				'type' => Controls_Manager::COLOR,
				'selectors' => [
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .insonis-post-tags .title',
			]
		);
		...
		$this->add_control(
			'show_links',
			[
				'label'     => __('Show Breadcrumb Links', 'insonis-themer'),
				'type'      => Controls_Manager::SWITCHER,
				'default'   => 'yes',
	...
	public function breadcrumbs(){
	 	$delimiter = '';
	 	$home = esc_html__('Home', 'insonis-themer');
	 	...
	 	if(!is_home() && !is_front_page() || is_paged()) {
			...
			if(is_category()){
			  	...
			  	$breadcrumb .= $before . esc_html__('Search results for', 'insonis-themer') . '"' . htmlentities(get_search_query()) . '"' . $after;
			  ...
			  $breadcrumb .= $before . esc_html__('Posts tagged', 'insonis-themer') . '"' . single_tag_title('', false) . '"' . $after;
			  	...
			  	if($userdata){
				 	$breadcrumb .= $before . esc_html__('Articles posted by', 'insonis-themer') . $userdata->display_name . $after;
			  	} 

			}elseif(is_404()){
			  	$breadcrumb .= $before . esc_html__('Error 404', 'insonis-themer') . $after;
			}

			if(get_query_var('paged')){
			  	$breadcrumb .= $before . esc_html__('Page','insonis-themer') . ' ' . get_query_var('paged') . $after;
			}
