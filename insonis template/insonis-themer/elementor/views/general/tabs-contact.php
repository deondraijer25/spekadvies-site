...
<?php if($settings['style'] == 'style-3'){ ?>
   <div class="tabs-contact-three">
      <div class="tabs-contact-three__wrap">
         <div class="tabs-contact-three__forms">
            ...
            <div class="tabs-contact-three__forms-inner">
               <ul class="nav tabs-contact-three__nav" id="tabs-<?php echo esc_attr($_rand) ?>" role="tablist">
                  <?php
                     $i = 0;
                     foreach ($settings['forms_content'] as $form){
                        $i++;
                        $active = $i == 1 ? ' active' : '';
                        echo '<li class="tabs-contact-three__nav-item" role="presentation">';
                           echo '<a class="tabs-contact-three__nav-link' . $active . '" data-bs-toggle="tab" href="#" data-bs-target="#tab-item-'. $_rand . $form['form_id'] . $i . '" role="tab">';
                              if( !empty($form['selected_icon']['value']) ){
                                 Icons_Manager::render_icon($form['selected_icon'], [ 'aria-hidden' => 'true' ]); 
                              }
                              echo esc_html($form['title']);
               ...
               <div class="tabs-contact-three__content tab-content">
                  <?php 
                     $i = 0;
                     foreach ($settings['forms_content'] as $form){
                        $i++;
                        $active = $i == 1 ? ' show active' : '';
                        echo '<div class="tabs-contact-three__panel tab-pane fade ' . $active . '" id="tab-item-'. $_rand . $form['form_id'] . $i . '" tabindex="' . esc_attr($i) . '">';
                           echo do_shortcode('[contact-form-7 id="' . $form['form_id'] . '" title="' . esc_attr($form['title']) . '"]');
                        echo '</div>';
                     }
                  ?>
         ...
         <?php 
            if( $settings['image']['url'] ){
               echo '<div class="tabs-contact-three__image">';
                  echo '<div class="tabs-contact-three__image-inner">';
                     echo '<img src="' . esc_url($settings['image']['url']) . '" alt="' . esc_attr($settings['title_text']) . '"/>';
                  echo '</div>';
                  echo '<div class="tabs-contact-three__arrow"></div>';
               echo '</div>';
            }
         ?> 
      </div>   
