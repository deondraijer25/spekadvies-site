(function($) {
  	...
  	var GaviasTheme = {
        ...
        if (window.innerWidth > body.clientWidth + 6) {
        	...
        	setTimeout(function(){
          	if($('body').hasClass('insonis-body-loading')){
              $('body').removeClass('insonis-body-loading');
              $('.insonis-page-loading').fadeOut(50);
          	}
      	}, 360);
	 	},
	 	...
	 	menuMobile: function(){
			...
			$( document ).on( 'click', '.close-canvas, .first-layer, .second-layer', function(e) {
			  	e.preventDefault();
			  	$('.gva-offcanvas-content').removeClass('open');
			  	$('#wp-main-content').removeClass('blur');
			})
		...
		other: function(){
			if( typeof Lenis !== 'undefined' && !document.body.classList.contains('elementor-editor-active') ) {
				var lenis = new Lenis({
					duration: 1.2,
			...
			$('.insonis-post-share.style-2 .btn-control-share').on('click', function(e){
				e.preventDefault();
				var wrapper = $(this).parents('.insonis-post-share');
